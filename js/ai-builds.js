...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderGridItems(items, targetContainer) {
    ...
    items.forEach(item => {
       ...
       const imgStr = `<img loading="lazy" src="${window.ItemCard?.image?.(item.id, 1) || ''}" class="w-full h-full object-contain drop-shadow-md transition-opacity" alt="${item.name}" data-image-fallback="item">`;
       ...
       btn.addEventListener('click', () => {
          ...
          if (iconTarget) {
              iconTarget.innerHTML = `<img loading="lazy" src="${window.ItemCard?.image?.(item.id, 1) || ''}" class="w-full h-full object-contain p-0.5" data-image-fallback="item">`;
          }
          if (textTarget) {
  ...
  function renderVisualSlot(slotName, itemData, iconClass) {
    ...
    return `
      <div class="relative w-10 h-10 md:w-12 md:h-12 bg-gradient-to-br from-[#1a2233] to-[#0a0d14] border border-gray-500 rounded shadow-lg flex items-center justify-center p-1 group shrink-0">
         <img loading="lazy" src="${window.ItemCard?.image?.(cleanId, 1) || ''}" class="w-full h-full object-contain drop-shadow-md group-hover:scale-110 transition-transform" alt="${itemData.name}" data-image-fallback="item">
    ...
    return `
      <div class="bg-albion-900 border border-gray-700 p-2 rounded-lg flex items-start space-x-2.5 hover:border-albion-accent/50 transition-colors shadow-inner w-full">
        <div class="relative w-8 h-8 md:w-10 md:h-10 bg-[#0a0d14] rounded border border-gray-800 flex items-center justify-center p-0.5 shrink-0 group mt-0.5">
          ${isConsumable ? `<i class="fa-solid ${fallbackIcon} text-sm md:text-base text-gray-400"></i>` : `<img loading="lazy" src="${window.ItemCard?.image?.(cleanId, 1) || ''}" class="w-full h-full object-contain group-hover:scale-110 transition-transform" data-image-fallback="item">
        `}
        </div>
