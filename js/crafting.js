          ...
          <div class="w-full text-left animate-fade-in">
            ...
            <div>
              ...
              <div class="grid grid-cols-2 gap-2">
                ${city.crafts.map(c => `<div class="bg-albion-900 border border-gray-600 rounded p-2 text-sm text-gray-200 flex items-center">${window.ItemCard?.inline?.(window.ItemCard.guessId(c, 6), c) || `<i class="fa-solid fa-hammer text-amber-600 mr-2 opacity-70"></i> ${c}`}</div>`).join('')}
              </div>
            </div>
          </div>
        `;
          ...
          ${outlandsBiomes.map(b => `
            <div class="biome-card bg-albion-800 border-2 border-gray-700 rounded-xl p-4 transition-all duration-300 flex flex-col" data-search="${b.searchTerms}">
              ...
              <ul class="text-xs text-gray-300 space-y-1 list-disc pl-4 mb-3">
                ${b.crafts.map(c => `<li>${window.ItemCard?.inline?.(window.ItemCard.guessId(c, 6), c) || c}</li>`).join('')}
              </ul>
