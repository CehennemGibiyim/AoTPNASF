    ...
    detailsArea.innerHTML = `
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <div class="bg-albion-900 border-2 border-albion-accent p-4 rounded-xl text-center shadow-lg">
          ...
          <img class="item-surface-icon" src="${window.ItemCard?.image?.(window.ItemCard.resourceId(biome.main.type, 6), 1, 64) || ''}" alt="${biome.main.type}" loading="lazy" data-image-fallback="item">
        ...
        <div class="bg-albion-900 border border-gray-700 p-4 rounded-xl text-center">
          ...
          <img class="item-surface-icon" src="${window.ItemCard?.image?.(window.ItemCard.resourceId(biome.sec1.type, 6), 1, 64) || ''}" alt="${biome.sec1.type}" loading="lazy" data-image-fallback="item">
          <div class="font-bold text-white">${biome.sec1.type}</div>
        </div>
        <div class="bg-albion-900 border border-gray-700 p-4 rounded-xl text-center">
          <div class="text-sm text-gray-400 mb-2"><span>${window.t('gathering-secResource', 'İkincil Kaynak')}</span></div>
          <img class="item-surface-icon" src="${window.ItemCard?.image?.(window.ItemCard.resourceId(biome.sec2.type, 6), 1, 64) || ''}" alt="${biome.sec2.type}" loading="lazy" data-image-fallback="item">
       ...
       const createMapCard = (m, isMain) => {
          ...
          const resHtml = `
            <div class="flex gap-2 mt-3 justify-center bg-black/30 p-2 rounded-lg border border-gray-700">
              <span class="flex flex-col items-center" title="${bData.main.type}"><img class="w-8 h-8 object-contain" src="${window.ItemCard?.image?.(window.ItemCard.resourceId(bData.main.type, m.tier), 1, 40) || ''}" alt="${bData.main.type}" loading="lazy" data-image-fallback="item"><span class="text-[10px] text-albion-accent font-bold uppercase tracking-wider">${getName(bData.main.type)}</span></span>
              <span class="w-px bg-gray-700 mx-1"></span>
              <span class="flex flex-col items-center" title="${bData.sec1.type}"><img class="w-8 h-8 object-contain" src="${window.ItemCard?.image?.(window.ItemCard.resourceId(bData.sec1.type, m.tier), 1, 40) || ''}" alt="${bData.sec1.type}" loading="lazy" data-image-fallback="item"><span class="text-[10px] text-gray-400 font-semibold uppercase tracking-wider">${getName(bData.sec1.type)}</span></span>
              <span class="w-px bg-gray-700 mx-1"></span>
              <span class="flex flex-col items-center" title="${bData.sec2.type}"><img class="w-8 h-8 object-contain" src="${window.ItemCard?.image?.(window.ItemCard.resourceId(bData.sec2.type, m.tier), 1, 40) || ''}" alt="${bData.sec2.type}" loading="lazy" data-image-fallback="item"><span class="text-[10px] text-gray-400 font-semibold uppercase tracking-wider">${getName(bData.sec2.type)}</span></span>
            </div>
          `;
