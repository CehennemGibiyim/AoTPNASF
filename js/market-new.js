...
function renderTable(data) {
  ...
  // Filter 0 prices and city
  let rows = data.filter(r => r.sell_price_min > 0 || r.buy_price_max > 0);
  if (cityF) rows = rows.filter(r => r.city === cityF || r.city.replace(' ','') === cityF.replace(' ',''));

  // Sorting
  if (sortCol) {
    rows.sort((a,b) => {
      let valA, valB;
      if (sortCol === 'item') { valA = getItemName(a.item_id); valB = getItemName(b.item_id); }
      else if (sortCol === 'city') { valA = a.city; valB = b.city; }
      else if (sortCol === 'sell') { valA = a.sell_price_min; valB = b.sell_price_min; }
      else if (sortCol === 'buy') { valA = a.buy_price_max; valB = b.buy_price_max; }
      else if (sortCol === 'qual') { valA = a.quality || 1; valB = b.quality || 1; }
      else if (sortCol === 'age') { valA = a.sell_price_min_date || a.buy_price_max_date; valB = b.sell_price_min_date || b.buy_price_max_date; }
    ...
    rows.sort((a,b) => {
      ...
      if (a.item_id > b.item_id) return 1;
      return (a.sell_price_min || Infinity) - (b.sell_price_min || Infinity);
    });
  }

  const count = document.getElementById('tableCount');
  const uniqueItems = new Set(rows.map(r => r.item_id)).size;
  if (count) count.textContent = `${uniqueItems} eşya`;

  const headers = lang === 'tr'
    ? [{k:'item',t:'Eşya'},{k:'city',t:'Şehir'},{k:'sell',t:'Satış Fiyatı'},{k:'buy',t:'Alış Emri'},{k:'qual',t:'Kalite'},{k:'age',t:'Veri Yaşı'}]
    : [{k:'item',t:'Item'},{k:'city',t:'City'},{k:'sell',t:'Sell Price'},{k:'buy',t:'Buy Order'},{k:'qual',t:'Quality'},{k:'age',t:'Data Age'}];

  let html = `<table class="price-table"><thead><tr>` + headers.map(h => 
    `<th onclick="setSort('${h.k}')" style="cursor:pointer; user-select:none;" class="sortable-th">
      ${h.t} <span style="color:var(--text-muted);font-size:10px">${sortCol===h.k ? (sortAsc?'▲':'▼') : '↕'}</span>
    </th>`
  ).join('') + `</tr></thead><tbody>`;

  let lastItemId = null;
  const showItemAlways = !!sortCol && sortCol !== 'item';
  ...
  rows.forEach(r => {
    ...
    const tier = getTier(r.item_id);

    const isFirstOfGroup = lastItemId !== r.item_id;
    lastItemId = r.item_id;

    html += `<tr>
      <td>${(isFirstOfGroup || showItemAlways) ? `<div class="item-cell"><img src="${RENDER}/${r.item_id}.png" onerror="this.style.display='none'" alt="${name}"/><div><div class="item-name">${name}</div><span class="item-tier">T${tier}</span></div></div>` : ''}</td>
